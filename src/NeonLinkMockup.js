...
import { Bell, Banknote, Briefcase, CalendarDays, ClipboardList, Clock3, File as FileIcon, FileBadge2, FileText, Flame, FolderKanban, HeartHandshake, Home, LogOut, MoreHorizontal, MessageSquare, Mic, Paperclip, Plus, Globe, User as UserIcon, Pin, Phone, Reply, Search, Send, Settings, Shield, Smile, Users, Video, } from "lucide-react";
...
import { FriendCategoryModal } from "@/components/friends/FriendCategoryModal";
...
function extractSurname(displayName) {
...
function normalizeSurnameToken(raw) {
    return raw
...
function shouldMergeSurnameTypo(a, b) {
...
function senderChatTextHex(senderId, selfId, selfColor, byId) {
    if (!senderId)
        return undefined;
    const raw = senderId === selfId ? (selfColor ?? byId[senderId]?.chatTextColor) : byId[senderId]?.chatTextColor;
...
function otherUserBubbleStyle(senderId, textHex) {
...
function SidebarUpcomingRow({ ev, navigate }) {
    ...
    const meetingVideoPath = ev.kind === "meeting" && ev.meetingId
        ? videoMeetingPath({
    ...
    const inner = (_jsxs(_Fragment, { children: [_jsx("div", { className: `h-10 w-10 shrink-0 rounded-2xl border flex items-center justify-center ${card.iconWrap}`, children: meetingVideoPath ? (_jsx(Video, { className: `h-5 w-5 ${card.clockClass}` })) : (_jsx(Clock3, { className: `h-5 w-5 ${card.clockClass}` })) }), _jsxs("div", { className: "min-w-0 flex-1", children: [_jsx("div", { className: "text-sm font-medium leading-snug break-words", children: ev.title }), _jsx("div", { className: "text-xs text-white mt-1 leading-snug break-words", children: sub }), meetingVideoPath ? (_jsx("div", { className: "text-[10px] text-cyan-200/80 mt-1", children: "Klick \u00F6ffnet Video in neuem Tab" })) : null] })] }));
    if (meetingVideoPath) {
        return (_jsx("a", { href: meetingVideoPath, target: "_blank", rel: "noopener noreferrer", className: `${rowClass} no-underline text-inherit cursor-pointer block`, "aria-label": `Meeting ${ev.title}, Video in neuem Tab`, children: inner }));
    }
    return (_jsx("button", { type: "button", onClick: () => void navigate("/kalender"), className: rowClass, children: inner }));
}
...
export default function NeonLinkMockup() {
    ...
    const [chatInsertPicker, setChatInsertPicker] = useState(null);
    const [friendCategoryOpen, setFriendCategoryOpen] = useState(false);
    const [friendCategoryCtx, setFriendCategoryCtx] = useState(null);
    const [voiceRecording, setVoiceRecording] = useState(false);
    const voiceRecorderRef = useRef(null);
    const voiceStreamRef = useRef(null);
    const voiceChunksRef = useRef([]);
    const socketRef = useRef(null);
    const joinedWorkspaceRoomsRef = useRef(new Set());
    const fileInputRef = useRef(null);
    const messageInputRef = useRef(null);
    const chatScrollEndRef = useRef(null);
    ...
    const prevActiveRoomForTypingRef = useRef(null);
    const loadFriendDataRef = useRef(async () => { });
    const playedNotificationIdsRef = useRef(new Set());
    const serverRoomsRef = useRef([]);
    ...
    const sidebarAppointmentsOnly = useMemo(() => sidebarUpcoming.filter((e) => e.kind !== "meeting"), [sidebarUpcoming]);
    const sidebarMeetingsOnly = useMemo(() => sidebarUpcoming.filter((e) => e.kind === "meeting"), [sidebarUpcoming]);
    ...
    const privateRooms = useMemo(() => sortByActivity(sourceRooms.filter((r) => r.workspaceId === workspaceScopeId && r.chatType === "private"), lastActivityByRoom), [sourceRooms, workspaceScopeId, lastActivityByRoom]);
    ...
    const visibleFriendsForSection = useMemo(() => friends.filter((friend) => {
    ...
    const markCalendarNewsSeen = (eventId) => {
        if (!currentUser)
            return;
    ...
    useEffect(() => {
        ...
        const t = window.setInterval(() => void loadFriendDataRef.current(uid), 45000);
    ...
    const queueFriendRequest = (toUserId) => {
        setFriendCategoryCtx({ mode: "send", toUserId });
        setFriendCategoryOpen(true);
    };
    const queueAcceptFriendRequest = (requestId) => {
        setFriendCategoryCtx({ mode: "accept", requestId });
        setFriendCategoryOpen(true);
    };
    const confirmFriendCategories = async (keys) => {
        if (!friendCategoryCtx || !currentUser)
            return;
        try {
            ...
            setFriendInfo(friendCategoryCtx.mode === "send"
    ...
    const setFriendGroupsForFriend = async (friendUserId, groups) => {
        if (!currentUser)
            return;
        try {
            const nextGroups = activeSection === "familie" && !groups.includes("familie")
                ? ["familie", ...groups]
                : groups;
    ...
    useEffect(() => {
        ...
        socket.on("connect", () => {
            ...
            if (uid)
                void loadFriendDataRef.current(uid);
        ...
        socket.on("friends:incomingRequest", (payload) => {
            if (payload.toUserId !== currentUserRef.current?.id)
                return;
        ...
        socket.on("chat:privateRoomUpsert", (payload) => {
            const room = payload?.room;
            if (!room?.id || room.chatType !== "private")
                return;
            if (room.workspaceId !== activeWorkspaceIdRef.current)
                return;
    ...
    useEffect(() => {
        const socket = socketRef.current;
        if (!socket?.connected || !isBackendOnline || !token)
            return;
    ...
    return (_jsxs("div", { className: "min-h-screen w-full bg-[#050816] text-white overflow-x-hidden lg:overflow-hidden", children: [_jsx("input", { ref: fileInputRef, type: "file", multiple: true, className: "hidden", onChange: onFilesSelected, accept: "image/*,.gif,video/*,audio/*,.zip,application/zip,application/x-zip-compressed" }), _jsx("div", { className: "absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.18),transparent_28%),radial-gradient(circle_at_top_right,rgba(168,85,247,0.16),transparent_22%),radial-gradient(circle_at_bottom_left,rgba(239,68,68,0.14),transparent_24%)]" }), _jsx("div", { className: "absolute inset-0 opacity-20 [background-image:linear-gradient(rgba(255,255,255,0.04)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.04)_1px,transparent_1px)] [background-size:32px_32px]" }), _jsxs("div", { className: "relative z-10 flex h-screen min-h-0 w-full max-w-full lg:min-w-0 lg:max-w-[100vw] overflow-x-auto overflow-y-hidden overscroll-x-contain touch-pan-y lg:overflow-hidden lg:touch-auto p-3 sm:p-4 gap-3 sm:gap-4", children: [_jsxs(motion.aside, { initial: { x: -20, opacity: 0 }, animate: { x: 0, opacity: 1 }, className: "w-24 shrink-0 rounded-3xl border border-cyan-400/20 bg-white/5 backdrop-blur-xl shadow-2xl shadow-cyan-500/10 flex flex-col items-center py-4 gap-4", children: [_jsx("div", { className: "h-14 w-14 rounded-2xl bg-gradient-to-br from-cyan-400 to-blue-600 flex items-center justify-center font-bold text-lg shadow-lg shadow-cyan-500/30", children: "NL" }), sections.map((s) => {
                                ...
                                return (_jsxs("button", { onClick: () => onSelectSection(s.id), className: `relative h-14 w-14 rounded-2xl flex items-center justify-center transition-all ${active
                                        ...
                                        : "bg-white/5 hover:bg-white/10"}`, title: s.label, "aria-label": `${s.label} Bereich waehlen`, children: [active && _jsx("span", { className: `absolute left-0 top-2 bottom-2 w-1 rounded-full bg-gradient-to-b ${theme.led}` }), _jsx(Icon, { className: `h-6 w-6 ${active ? "text-cyan-100" : "text-white"}` })] }, s.id));
                            }), _jsxs("div", { className: "mt-auto flex flex-col gap-3", children: [_jsxs("button", { type: "button", onClick: () => setMainView("news"), className: `relative h-12 w-12 rounded-xl bg-white/5 flex items-center justify-center hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/50 ${newsBellCount > 0 ? "animate-neonlink-bell" : ""}`, "aria-label": `Neuigkeiten${newsBellCount > 0 ? `, ${newsBellCount} ungelesen` : ""}`, children: [_jsx(Bell, { className: `h-5 w-5 ${newsBellCount > 0 ? "text-cyan-100" : "text-white"}` }), newsBellCount > 0 ? (_jsx("span", { className: "absolute -top-0.5 -right-0.5 min-w-[1.125rem] h-[1.125rem] px-0.5 rounded-full bg-rose-500 text-[10px] font-bold text-white flex items-center justify-center leading-none shadow-lg shadow-rose-500/40 border border-rose-300/60", children: newsBellCount > 99 ? "99+" : newsBellCount })) : null] }), _jsx("button", { type: "button", onClick: () => setProfileModalOpen(true), className: "h-12 w-12 rounded-xl bg-white/5 flex items-center justify-center hover:bg-white/10", "aria-label": "Einstellungen", children: _jsx(Settings, { className: "h-5 w-5 text-white" }) })] })] }), _jsxs(motion.aside, { initial: { opacity: 0, y: 16 }, animate: { opacity: 1, y: 0 }, className: "w-80 min-w-0 max-w-[20rem] shrink-0 overflow-y-auto overflow-x-hidden rounded-3xl border border-white/10 bg-white/5 backdrop-blur-xl p-4 flex flex-col gap-4 shadow-2xl shadow-black/20", children: [_jsxs("div", { className: "flex items-center justify-between gap-2 min-w-0", children: [_jsxs("div", { className: "min-w-0", children: [_jsx("div", { className: "text-xs uppercase tracking-[0.25em] text-cyan-100 font-semibold", children: "NeonLink" }), workspaceHeaderLabel ? (_jsx("div", { className: "text-[11px] text-white mt-0.5 whitespace-normal break-words leading-snug", title: workspaceHeaderLabel, children: workspaceHeaderLabel })) : null, _jsx("div", { className: "text-2xl font-semibold mt-1 truncate", children: activeSectionData.label })] }), _jsx(Shield, { className: "h-5 w-5 text-cyan-300" })] }), _jsxs("div", { className: "relative", children: [_jsx(Search, { className: "h-4 w-4 absolute left-3 top-1/2 -translate-y-1/2 text-white" }), _jsx(Input, { value: chatSidebarSearch, onChange: (e) => setChatSidebarSearch(e.target.value), className: "pl-9 bg-white/5 border-white/10 rounded-2xl h-11 text-white placeholder:text-white/60", placeholder: "Chats filtern\u2026", "aria-label": "Chats filtern" })] }), _jsx(Card, { className: `bg-gradient-to-br ${activeTheme.border} ${activeTheme.panel} rounded-3xl text-white`, children: _jsxs(CardContent, { className: "p-4", children: [_jsx("div", { className: `text-sm ${activeTheme.chip}`, children: "Aktiver Bereich" }), _jsx("div", { className: "text-lg font-semibold mt-1", children: activeSectionData.label }), _jsx("div", { className: "text-xs text-white mt-2", children: "Chats, Meetings, Dateien und gemeinsame Planung in einem Bereich." })] }) }), _jsxs("div", { children: [_jsxs("div", { className: "flex items-center justify-between gap-2 mb-3 min-w-0", children: [_jsx("div", { className: "text-sm font-semibold uppercase tracking-[0.2em] text-white shrink-0", children: "Chats" }), _jsxs(Button, { type: "button", onClick: () => setNewChatOpen(true), className: "h-8 shrink-0 rounded-xl border border-cyan-400/35 bg-cyan-500/15 text-cyan-100 hover:bg-cyan-500/25 text-xs px-2.5", children: [_jsx(Plus, { className: "h-3.5 w-3.5 inline-block mr-1 -mt-0.5 align-middle" }), "Neu"] })] }), _jsxs("div", { className: "space-y-5", children: [_jsxs("div", { children: [_jsxs("div", { className: "flex items-center gap-2 text-[11px] font-semibold text-white mb-2 uppercase tracking-wider", children: [_jsx(Globe, { className: "h-3.5 w-3.5 text-cyan-300" }), "Hauptchat"] }), _jsxs("div", { className: "space-y-2", children: [globalRooms.length > 0 && globalRoomsFiltered.length === 0 ? (_jsxs("p", { className: "text-xs text-amber-100 font-semibold px-1", children: ["Kein Treffer f\u00FCr \u201E", chatSidebarSearch.trim(), "\u201C."] })) : null, globalRoomsFiltered.map((room) => (_jsx(motion.button, { whileHover: { x: 4 }, type: "button", onClick: () => selectChatRoom(room.id), className: `w-full rounded-2xl p-3 text-left border ${room.id === activeRoom?.id
                                                                    ? `${activeTheme.border} bg-white/10`
                                                                    : "border-white/10 bg-white/5"}`, children: _jsxs("div", { className: "flex items-start justify-between gap-2 min-w-0", children: [_jsxs("div", { className: "min-w-0 flex-1", children: [_jsxs("div", { className: "flex items-center gap-2 min-w-0", children: [_jsx("span", { className: "font-medium truncate", children: displayChatTitle(room, senderUserId, userNamesById) }), room.kind !== "text" ? (_jsx("span", { className: "text-[10px] text-white uppercase shrink-0", children: room.kind })) : null] }), previewByRoomId[room.id] ? (_jsx("div", { className: "text-[11px] text-white truncate mt-0.5", children: previewByRoomId[room.id] })) : null] }), _jsxs("div", { className: "flex flex-col items-end gap-1 shrink-0", children: [(() => {
                                                                                ...
                                                                                })(), _jsx(MessageSquare, { className: "h-4 w-4 text-white" })] })] }) }, room.id)))] })] }), _jsxs("div", { children: [_jsxs("div", { className: "flex items-center gap-2 text-[11px] font-semibold text-white mb-2 uppercase tracking-wider", children: [_jsx(Users, { className: "h-3.5 w-3.5 text-fuchsia-300" }), "Gruppen"] }), _jsx("div", { className: "space-y-2", children: groupRooms.length === 0 ? (_jsx("p", { className: "text-xs text-white px-1", children: "Noch keine Gruppen in diesem Bereich." })) : groupRoomsFiltered.length === 0 ? (_jsxs("p", { className: "text-xs text-amber-100 font-semibold px-1", children: ["Kein Treffer f\u00FCr \u201E", chatSidebarSearch.trim(), "\u201C."] })) : (groupRoomsFiltered.map((room) => (_jsx(motion.button, { whileHover: { x: 4 }, type: "button", onClick: () => selectChatRoom(room.id), className: `w-full rounded-2xl p-3 text-left border ${room.id === activeRoom?.id
                                                                ? `${activeTheme.border} bg-white/10`
                                                                : "border-white/10 bg-white/5"}`, children: _jsxs("div", { className: "flex items-start justify-between gap-2 min-w-0", children: [_jsxs("div", { className: "min-w-0 flex-1", children: [_jsx("div", { className: "font-medium truncate", children: room.name }), previewByRoomId[room.id] ? (_jsx("div", { className: "text-[11px] text-white truncate mt-0.5", children: previewByRoomId[room.id] })) : null] }), _jsxs("div", { className: "flex flex-col items-end gap-1 shrink-0", children: [(() => {
                                                                            ...
                                                                            })(), _jsx(MessageSquare, { className: "h-4 w-4 text-white" })] })] }) }, room.id)))) })] }), _jsxs("div", { children: [_jsxs("div", { className: "flex items-center gap-2 text-[11px] font-semibold text-white mb-2 uppercase tracking-wider", children: [_jsx(UserIcon, { className: "h-3.5 w-3.5 text-emerald-300" }), "Privat", _jsx("span", { className: "normal-case font-normal text-white/40 font-sans", children: "\u00B7 alle Rubriken" })] }), _jsx("div", { className: "space-y-2", children: privateRooms.length === 0 ? (_jsx("p", { className: "text-xs text-white px-1", children: "Keine Direktchats \u2014 \u201ENeu\u201C nutzen." })) : privateRoomsFiltered.length === 0 ? (_jsxs("p", { className: "text-xs text-amber-100 font-semibold px-1", children: ["Kein Treffer f\u00FCr \u201E", chatSidebarSearch.trim(), "\u201C."] })) : (privateRoomsFiltered.map((room) => (_jsx(motion.button, { whileHover: { x: 4 }, type: "button", onClick: () => selectChatRoom(room.id), className: `w-full rounded-2xl p-3 text-left border ${room.id === activeRoom?.id
                                                                ? `${activeTheme.border} bg-white/10`
                                            ...
                                            return (_jsxs("button", { type: "button", onClick: () => {
                                                ...
                                                if (!res.ok) {
                                                    ...
                                                    setAppNotice(d.error === "not_found"
                                                ...
                                                if (!res.ok)
                                            ...
                                            } }) })) : (_jsxs("div", { className: "lg:col-span-8 min-w-0 flex flex-col gap-3 min-h-0 lg:h-full lg:max-h-full", children: [_jsx(DeparturesDashboard, {}), _jsxs(Card, { className: "flex-1 min-h-0 min-w-0 rounded-3xl border border-white/10 bg-white/5 text-white backdrop-blur-xl flex flex-col lg:min-h-0", children: [_jsx(CardHeader, { className: "rounded-t-3xl border-b border-white/10 px-6 sm:px-10 pt-7 pb-4 flex justify-center items-center shrink-0", children: _jsx(CardTitle, { className: "text-lg sm:text-xl text-center font-semibold tracking-wide text-white leading-normal", children: "Live-Chat" }) }), _jsx(CardContent, { className: "flex-1 min-h-0 min-w-0 rounded-b-3xl p-0 flex flex-col overflow-hidden", children: _jsxs("div", { ref: chatScrollContainerRef, className: "flex-1 min-h-0 overflow-y-auto overflow-x-hidden p-4 flex flex-col gap-4", children: [sourceRooms.length === 0 && activeWorkspaceId ? (_jsx("div", { className: "rounded-2xl border border-white/10 bg-white/5 px-4 py-8 text-sm text-white text-center leading-relaxed", children: "In diesem Workspace sind noch keine Raeume geladen \u2014 bitte Seite neu laden oder kurz warten. Ohne Backend bleibt die Liste leer." })) : null, !activeWorkspaceId && token ? (_jsx("div", { className: "rounded-2xl border border-cyan-400/20 bg-cyan-500/10 px-4 py-8 text-sm text-cyan-50 font-semibold text-center", children: "Workspace wird geladen \u2026" })) : null, pinnedMessage ? (_jsxs("div", { className: "rounded-2xl border border-yellow-300/30 bg-yellow-500/10 p-3 text-xs", children: [_jsxs("div", { className: "flex items-center gap-2 text-yellow-200", children: [_jsx(Pin, { className: "h-3 w-3" }), " Angepinnt"] }), _jsx("div", { className: "mt-1 text-white", children: pinnedMessage.calendarAnnouncement ? (_jsx(CalendarAnnouncementMessage, { payload: pinnedMessage.calendarAnnouncement })) : (pinnedMessage.text) })] })) : null, roomMessages.map((m, i) => {
                                                                    const msgAvatarSrc = m.senderUserId && m.senderUserId === currentUser?.id
                                                                        ? currentUser?.avatarUrl
                                                                        : m.senderUserId
                                                                            ? usersById[m.senderUserId]?.avatarUrl ?? m.avatarUrl
                                                                            : m.avatarUrl;
                                                                    const openSenderProfile = Boolean(m.senderUserId && m.senderUserId !== currentUser?.id);
                                                                    const senderHex = senderChatTextHex(m.senderUserId, currentUser?.id, currentUser?.chatTextColor, usersById);
                                                                    const mineId = currentUser?.id;
                                                                    const isMine = Boolean(mineId && (m.senderUserId === mineId || (!m.senderUserId && m.from === "Du")));
                                                                    const peerBubble = !isMine && !m.calendarAnnouncement
                                                                        ? otherUserBubbleStyle(m.senderUserId, senderHex)
                                                                        : null;
                                                                    const groupMax = "max-w-[min(92%,26rem)]";
                                                                    const bubbleMax = "max-w-full";
                                                                    return (_jsx(motion.div, { initial: { opacity: 0, y: 10 }, animate: { opacity: 1, y: 0 }, transition: { delay: i * 0.08 }, className: `group flex w-full ${isMine ? "justify-end" : "justify-start"}`, children: _jsxs("div", { className: `flex gap-2 min-w-0 items-end ${groupMax} ${isMine ? "flex-row-reverse" : "flex-row"}`, children: [openSenderProfile ? (_jsx("button", { type: "button", onClick: () => setPeerProfileUserId(m.senderUserId), className: "relative h-10 w-10 shrink-0 overflow-hidden rounded-full border-2 border-cyan-400/35 p-0 bg-transparent cursor-pointer hover:ring-2 hover:ring-cyan-400/50 transition-shadow shadow-[0_0_14px_rgba(34,211,238,0.2)]", style: peerBubble ? { borderColor: peerBubble.borderColor } : undefined, title: "Profil anzeigen", children: msgAvatarSrc ? (_jsx("img", { src: msgAvatarSrc, alt: "", className: "h-full w-full object-cover" })) : (_jsx("span", { className: "neonlink-avatar-initials text-[12px] leading-none", children: m.from.slice(0, 2).toUpperCase() })) })) : (_jsx(Avatar, { className: "relative h-10 w-10 shrink-0 overflow-hidden rounded-full border border-cyan-400/30 shadow-[0_0_12px_rgba(34,211,238,0.18)]", children: msgAvatarSrc ? (_jsx("img", { src: msgAvatarSrc, alt: "", className: "h-full w-full object-cover" })) : (_jsx(AvatarFallback, { className: "neonlink-avatar-initials text-[12px] leading-none", children: m.from.slice(0, 2).toUpperCase() })) })), _jsxs("div", { className: `flex-1 min-w-0 flex flex-col ${isMine ? "items-end" : "items-stretch"}`, children: [_jsxs("div", { className: `flex flex-wrap items-center gap-2 text-sm w-full ${isMine ? "justify-end" : ""}`, children: [openSenderProfile ? (_jsx("button", { type: "button", onClick: () => setPeerProfileUserId(m.senderUserId), className: `font-semibold text-left transition-colors ${senderHex ? "hover:opacity-90" : "hover:text-cyan-200"}`, style: senderHex ? { color: senderHex } : undefined, children: m.from })) : (_jsx("span", { className: `font-semibold ${senderHex ? "" : "text-white"}`, style: senderHex ? { color: senderHex } : undefined, children: m.from })), _jsx("span", { className: `h-2 w-2 rounded-full ${m.role === "online"
                                                                                                        ? "bg-emerald-400"
                                                                                                        : m.role === "alert"
                                                                                                            ? "bg-red-400 animate-pulse"
                                                                                                            : m.role === "admin"
                                                                                                                ? "bg-cyan-400"
                                                                                                                : "bg-yellow-400"}` }), _jsx("span", { className: "text-white", children: m.time })] }), _jsxs("div", { className: `mt-1 w-full min-w-0 max-w-full rounded-2xl border px-4 py-3 text-sm font-medium leading-relaxed shadow-lg shadow-black/10 break-words [overflow-wrap:anywhere] ${isMine
                                                                                                ? "border-cyan-300/50 bg-gradient-to-br from-cyan-500/35 via-cyan-600/20 to-slate-900/55"
                                                                                                : m.calendarAnnouncement
                                                                                                    ? "border-white/10 bg-white/5 text-white"
                                                                                                    : peerBubble
                                                                                                        ? "text-white"
                                                                                                        : "border-white/10 bg-white/[0.07] text-white"} ${isMine && !senderHex && !m.calendarAnnouncement ? "text-white" : ""}`, style: {
                                                                                                ...
                                                                                                ...(peerBubble
                                                                                                    ...
                                                                                                    : {}),
                                                                                            }, children: [m.replyTo ? (_jsxs("div", { className: "mb-2 min-w-0 rounded-lg border border-white/10 bg-black/20 px-2 py-1 text-[11px] text-white break-words [overflow-wrap:anywhere]", children: [_jsx("span", { className: "text-cyan-100 font-semibold", children: m.replyTo.from }), _jsx("span", { className: "text-white", children: " \u00B7 " }), m.replyTo.preview] })) : null, m.calendarAnnouncement ? (_jsx(CalendarAnnouncementMessage, { payload: m.calendarAnnouncement })) : (_jsx(ChatMessageText, { text: m.text, className: "block whitespace-pre-wrap break-words [overflow-wrap:anywhere]" }))] }), m.attachments?.length ? (_jsx("div", { className: `mt-2 space-y-2 w-full ${bubbleMax} ${isMine ? "ml-auto" : ""}`, children: m.attachments.map((att) => (_jsxs("div", { className: "rounded-xl border border-cyan-400/25 bg-cyan-500/10 p-2 text-xs", children: [_jsxs("div", { className: "flex items-center gap-2 min-w-0 mb-1", children: [_jsx(FileIcon, { className: "h-4 w-4 text-cyan-200 shrink-0" }), _jsx("div", { className: "min-w-0 truncate font-medium", children: att.fileName }), _jsx("span", { className: "text-[10px] text-white/70 shrink-0", children: formatBytes(att.sizeBytes) })] }), _jsx(ChatAttachmentMedia, { attachmentId: att.id, mimeType: att.mimeType, fileName: att.fileName, token: token })] }, att.id))) })) : !m.calendarAnnouncement &&
                                                                                            (m.text.toLowerCase().includes("zip") || m.text.toLowerCase().includes("bilder")) ? (_jsxs("div", { className: `mt-2 rounded-xl border border-white/10 bg-white/5 p-2 text-xs flex items-center gap-2 ${bubbleMax} ${isMine ? "ml-auto" : ""}`, children: [_jsx(FileIcon, { className: "h-4 w-4 text-cyan-200" }), _jsxs("div", { children: [_jsx("div", { children: "Anhang vorhanden (Demo)" }), _jsx("div", { className: "text-white", children: "Bild/Video/Musik/ZIP" })] })] })) : null, m.reactions && m.reactions.length > 0 ? (_jsx("div", { className: `flex flex-wrap gap-1 mt-1.5 w-full ${isMine ? "justify-end" : ""}`, children: [...new Set(m.reactions.map((r) => r.emoji))].map((emoji) => {
                                                                                                const count = m.reactions.filter((r) => r.emoji === emoji).length;
                                                                                                return (_jsxs("button", { type: "button", onClick: () => void toggleMessageReaction(m.id, emoji), className: "rounded-full border border-white/15 bg-white/10 px-2 py-0.5 text-[12px] hover:bg-white/20 leading-none", children: [emoji, count > 1 ? (_jsx("span", { className: "text-[10px] ml-0.5 opacity-80", children: count })) : null] }, emoji));
                                                                                            }) })) : null, _jsxs("div", { className: `mt-1 items-center gap-2 text-[11px] text-white w-full ${isMine ? "justify-end" : ""} ${reactionBarForMessageId === m.id || messageOverflowId === m.id
                                                                                                ? "flex"
                                                                                                : "hidden group-hover:flex"}`, children: [_jsxs("button", { type: "button", onClick: () => setReplyToMessageId(m.id), className: "hover:text-white flex items-center gap-1", children: [_jsx(Reply, { className: "h-3 w-3" }), " Antworten"] }), _jsx("button", { type: "button", onClick: (e) => {
                                                                                                        ...
                                                                                                        setMessageOverflowId(null);
                                                                                                                ...
                                                                                                                setReactionBarForMessageId(null);
                                                                                                            }, className: "hover:text-white", "aria-expanded": messageOverflowId === m.id, "aria-label": "Weitere Aktionen", children: _jsx(MoreHorizontal, { className: "h-3 w-3" }) }), messageOverflowId === m.id ? (_jsx("div", { className: `absolute ${isMine ? "right-0" : "left-0"} top-full z-20 mt-1 min-w-[10.5rem] rounded-xl border border-white/15 bg-[#0f172a] py-1 shadow-xl shadow-black/50`, onClick: (e) => e.stopPropagation(), onKeyDown: (e) => e.stopPropagation(), children: _jsx("button", { type: "button", className: "w-full text-left px-3 py-2 text-xs text-white hover:bg-white/10", onClick: () => {
                                                                                                                    ...
                                                                                                                    setMessageOverflowId(null);
                                                                                                                }, children: "Text kopieren" }) })) : null] })] }), reactionBarForMessageId === m.id ? (_jsx("div", { className: `mt-1.5 flex flex-wrap gap-1 w-full ${isMine ? "justify-end" : ""}`, children: CHAT_QUICK_REACTIONS.map((emoji) => (_jsx("button", { type: "button", className: "rounded-lg border border-white/10 bg-white/10 px-2 py-0.5 text-base leading-none hover:bg-white/20", onClick: (e) => {
                                                                                                    e.stopPropagation();
                                                                                                    void toggleMessageReaction(m.id, emoji);
                                                                                                    setReactionBarForMessageId(null);
                                                                                                }, children: emoji }, emoji))) })) : null] })] }) }, m.id));
                                                                }), typingOthersLabel ? (_jsx("div", { className: "text-xs text-emerald-100 font-semibold animate-pulse", children: typingOthersLabel })) : null, messageInput.trim().length > 0 ? (_jsx("div", { className: "text-xs text-cyan-100 font-semibold", children: "Du tippst gerade..." })) : null, _jsx("div", { ref: chatScrollEndRef, className: "h-px w-full shrink-0", "aria-hidden": true })] }) }), _jsxs("div", { className: "border-t border-white/10 p-4", children: [replyToMessageId ? (_jsxs("div", { className: "mb-2 text-xs text-cyan-200 rounded-lg border border-cyan-400/20 bg-cyan-500/10 p-2", children: ["Antwort auf ", replyTarget?.from ?? "Nachricht", replyTarget ? (_jsxs("span", { className: "text-white", children: [" \u00B7 ", replyTarget.text.slice(0, 80), replyTarget.text.length > 80 ? "…" : ""] })) : null, _jsx("button", { type: "button", onClick: () => setReplyToMessageId(null), className: "ml-2 text-white hover:text-white", children: "abbrechen" })] })) : null, pendingAttachments.length > 0 ? (_jsx("div", { className: "mb-2 flex flex-wrap gap-2", children: pendingAttachments.map((a, idx) => (_jsxs("span", { className: "text-[11px] rounded-full border border-white/15 bg-white/10 px-2 py-1 flex items-center gap-1", children: [a.fileName, _jsx("button", { type: "button", className: "text-white hover:text-white", onClick: () => setPendingAttachments((prev) => prev.filter((_, i) => i !== idx)), children: "\u00D7" })] }, `${a.fileName}-${idx}`))) })) : null, _jsxs("div", { className: "rounded-2xl border border-cyan-300/20 bg-[#0b1128]/80 px-2 py-2 sm:px-3 flex flex-wrap sm:flex-nowrap items-stretch sm:items-center gap-2 min-w-0 max-w-full", children: [_jsx(Button, { type: "button", variant: "ghost", onClick: openFilePicker, className: "rounded-xl text-white hover:text-white hover:bg-white/10 shrink-0 h-10 w-10 p-0 inline-flex items-center justify-center", "aria-label": "Datei anhaengen", children: _jsx(Paperclip, { className: "h-5 w-5" }) }), _jsx(Button, { type: "button", variant: "ghost", onClick: () => (voiceRecording ? stopVoiceRecording() : void startVoiceRecording()), className: `rounded-xl shrink-0 h-10 w-10 p-0 inline-flex items-center justify-center ${voiceRecording
                                                                            ? "bg-red-500/35 text-red-100 border border-red-400/40 animate-pulse"
                                                                            : "text-white hover:text-white hover:bg-white/10"}`, "aria-label": voiceRecording ? "Sprachaufnahme beenden" : "Sprachnachricht aufnehmen", children: _jsx(Mic, { className: "h-5 w-5" }) }), _jsx("input", { ref: messageInputRef, className: "min-w-0 flex-1 basis-[8rem] sm:basis-auto bg-transparent outline-none text-sm placeholder:text-white/60 px-1 py-2", placeholder: "Nachricht \u2026 (Enter zum Senden)", value: messageInput, onChange: (e) => setMessageInput(e.target.value), onKeyDown: (e) => {
                                                                            if (e.key === "Enter") {
                                                                                void sendMessage();
                                                                            }
                                                                                ...
                                                                                }, className: "rounded-xl text-white hover:text-white hover:bg-white/10 h-10 w-10 p-0 inline-flex items-center justify-center", "aria-label": "Emoji oder GIF einf\u00FCgen", "aria-expanded": Boolean(chatInsertPicker), children: _jsx(Smile, { className: "h-5 w-5" }) }), chatInsertPicker ? (_jsxs("div", { className: "absolute bottom-full right-0 mb-2 z-30 w-[min(100vw-2rem,22rem)] max-h-[min(70vh,22rem)] flex flex-col rounded-xl border border-white/15 bg-[#0f172a] shadow-xl shadow-black/50 overflow-hidden", onClick: (e) => e.stopPropagation(), children: [_jsxs("div", { className: "flex gap-1 p-2 border-b border-white/10 shrink-0", children: [_jsx("button", { type: "button", className: `flex-1 rounded-lg py-1.5 text-xs font-medium ${chatInsertPicker === "emoji"
                                                                                                    ? "bg-cyan-500/25 text-cyan-100"
                                                                                                    : "text-white/60 hover:bg-white/10"}`, onClick: () => setChatInsertPicker("emoji"), children: "Emoji" }), _jsx("button", { type: "button", className: `flex-1 rounded-lg py-1.5 text-xs font-medium ${chatInsertPicker === "gif"
                                                                                                    ? "bg-violet-500/25 text-violet-100"
                                                                                                    : "text-white/60 hover:bg-white/10"}`, onClick: () => setChatInsertPicker("gif"), children: "GIF" })] }), _jsx("div", { className: "overflow-y-auto p-2 min-h-0", children: chatInsertPicker === "emoji" ? (_jsx("div", { className: "space-y-3", children: CHAT_EMOJI_SECTIONS.map((section) => (_jsxs("div", { children: [_jsx("div", { className: "text-[10px] uppercase tracking-wide text-white/40 mb-1.5", children: section.title }), _jsx("div", { className: "flex flex-wrap gap-1", children: section.emojis.map((emoji) => (_jsx("button", { type: "button", className: "rounded-lg border border-white/10 bg-white/10 px-2 py-1 text-lg leading-none hover:bg-white/20", onClick: () => {
                                                                                                                setMessageInput((p) => `${p}${emoji}`);
                                                                                                                setChatInsertPicker(null);
                                                                                                                focusMessageInput();
                                                                                                            }, children: emoji }, `${section.title}-${emoji}`))) })] }, section.title))) })) : (_jsx("div", { className: "grid grid-cols-3 gap-2", children: CHAT_PRESET_GIFS.map((g) => (_jsx("button", { type: "button", className: "rounded-lg overflow-hidden border border-white/15 bg-black/30 hover:border-cyan-400/40 transition-colors", title: g.label, onClick: () => {
                                                                                                    setMessageInput(g.url);
                                                                                                    setChatInsertPicker(null);
                                                                                                    focusMessageInput();
                                                                                                }, children: _jsx("img", { src: g.url, alt: g.label, className: "w-full h-14 object-cover", loading: "lazy" }) }, g.url))) })) })] })) : null, _jsxs(Button, { type: "button", className: "rounded-xl bg-cyan-500/20 border border-cyan-300/30 text-cyan-100 hover:bg-cyan-500/30 inline-flex items-center justify-center gap-2 px-4 py-2.5 sm:py-2 text-sm whitespace-nowrap shrink-0", onClick: () => {
                                                                                    void sendMessage();
                                                    ...
                                                    : "border-violet-400/20 bg-white/5 hover:bg-white/[0.07]"} text-white backdrop-blur-xl overflow-hidden`, children: _jsxs("div", { className: "p-4 sm:p-5 flex gap-4 items-start min-w-0", children: [_jsx("div", { className: "relative shrink-0", children: _jsx(Avatar, { className: `h-[5rem] w-[5rem] sm:h-[5.25rem] sm:w-[5.25rem] overflow-hidden rounded-2xl border-2 ${myEffectivePresence === "on_call"
                                                                    ...
                                                                    : "border-violet-400/35 ring-2 ring-cyan-500/10"}`, children: currentUser?.avatarUrl ? (_jsx("img", { src: currentUser.avatarUrl, alt: "", className: "h-full w-full object-cover" })) : (_jsx(AvatarFallback, { className: "neonlink-avatar-initials neonlink-avatar-initials-lg text-xl leading-none", children: (currentUser?.displayName ?? "?").slice(0, 2).toUpperCase() })) }) }), _jsxs("div", { className: "min-w-0 flex-1 space-y-2", children: [_jsxs("div", { className: "flex items-center justify-between gap-2", children: [_jsxs("span", { className: "text-xs uppercase tracking-wider text-violet-100 font-semibold flex items-center gap-1.5", children: [_jsx(UserIcon, { className: "h-3.5 w-3.5" }), "Dein Profil"] }), _jsx(StatusPill, { presence: myEffectivePresence })] }), _jsx("div", { className: "text-lg font-semibold truncate", children: currentUser?.displayName ?? "Gast" }), currentUser?.statusMessage ? (_jsxs("p", { className: "text-xs text-white line-clamp-2 leading-snug", children: ["\u201E", currentUser.statusMessage, "\u201C"] })) : (_jsx("p", { className: "text-xs text-white", children: "Kein Statustext \u2014 im Profil erg\u00E4nzen." })), _jsxs("div", { className: "text-[11px] text-white space-y-0.5", children: [_jsxs("div", { className: "flex items-center gap-2 flex-wrap", children: [_jsx("span", { className: "text-white", children: "E-Mail (Profil):" }), _jsx("span", { className: "truncate", children: currentUser?.contactEmail ?? "—" })] }), _jsxs("div", { className: "flex items-center gap-2 flex-wrap", children: [_jsx("span", { className: "text-white", children: "Telefon:" }), _jsx("span", { children: currentUser?.phoneDigits
                                                                                        ? maskPhoneDigits(currentUser.phoneDigits)
                                                                                        : "—" })] })] }), _jsxs("div", { className: "pt-1 space-y-0.5", children: [_jsx("span", { className: "text-xs font-semibold text-cyan-100", children: "Profil bearbeiten \u2192" }), _jsx("p", { className: "text-[10px] text-white leading-snug", children: "Chat-Schriftfarbe: im Profil-Dialog direkt unter dem Profilbild (t\u00FCrkiser Block)." })] })] })] }) }), _jsxs("div", { className: "flex flex-col gap-4 min-h-0 lg:flex-1 lg:min-h-0 lg:overflow-y-auto lg:overflow-x-hidden lg:overscroll-y-contain [scrollbar-gutter:stable] pr-0.5", children: [_jsxs(Card, { className: "rounded-3xl border-white/10 bg-white/5 text-white backdrop-blur-xl shrink-0", children: [_jsx(CardHeader, { children: _jsxs(CardTitle, { className: "text-lg", children: ["Status (", activeSectionData.label, ")"] }) }), _jsxs(CardContent, { className: "space-y-3", children: [visibleFriendsForSection.length === 0 ? (_jsx("div", { className: "rounded-2xl border border-white/10 bg-white/5 p-3 text-xs text-white", children: "Keine Freunde in dieser Gruppe." })) : null, visibleFriendsForSection.map((friend) => {
                                                                        ...
                                                                        return (_jsxs("div", { className: `rounded-2xl border p-3 flex gap-3 items-start transition-colors ${fp === "on_call"
                                                                                ...
                                                                                : "border-emerald-400/20 bg-emerald-500/10"}`, children: [_jsx(Avatar, { className: "h-9 w-9 shrink-0 overflow-hidden rounded-full border border-emerald-300/25", children: friend.avatarUrl ? (_jsx("img", { src: friend.avatarUrl, alt: "", className: "h-full w-full object-cover" })) : (_jsx(AvatarFallback, { className: "neonlink-avatar-initials-emerald text-[11px] leading-none", children: friend.displayName.slice(0, 2).toUpperCase() })) }), _jsxs("div", { className: "min-w-0 flex-1", children: [_jsxs("div", { className: "flex flex-wrap items-center gap-2", children: [_jsxs("div", { className: "text-sm text-emerald-200", children: [friend.displayName, " ", _jsx("span", { title: option?.label ?? labelGroup, children: option?.emoji ?? "👤" })] }), _jsx(StatusPill, { presence: fp })] }), friend.statusMessage ? (_jsx("div", { className: "text-[11px] text-white mt-1 line-clamp-2", children: friend.statusMessage })) : null, friend.contactEmail ? (_jsxs("div", { className: "text-[10px] text-white mt-1 truncate", children: ["\u2709 ", friend.contactEmail] })) : null, friend.phoneMasked ? (_jsxs("div", { className: "text-[10px] text-white truncate", children: ["\uD83D\uDCDE ", friend.phoneMasked] })) : null, _jsx("div", { className: "text-[10px] text-white mt-1", children: fg.length > 1 ? `${option?.label ?? labelGroup} (+${fg.length - 1})` : option?.label ?? labelGroup })] })] }, friend.id));
                                                                    })] })] }), _jsxs(Card, { className: "rounded-3xl border-white/10 bg-white/5 text-white backdrop-blur-xl shrink-0", children: [_jsxs(CardHeader, { children: [_jsx(CardTitle, { className: "text-lg", children: "N\u00E4chste Termine" }), _jsxs("p", { className: "text-[11px] text-white font-normal mt-1 leading-snug", children: ["Kalender-Eintr\u00E4ge aus allen Workspaces \u00B7 Farbe = Rubrik (z.\u00A0B. Feuerwehr rot, Familie t\u00FCrkis). Video-Meetings stehen separat unter ", _jsx("strong", { className: "text-white", children: "Meetings" }), "."] })] }), _jsx(CardContent, { className: "space-y-3", children: !token || !currentUser ? (_jsx("div", { className: "text-xs text-white py-4 text-center", children: "Anmelden, um Termine zu sehen." })) : sidebarAppointmentsOnly.length === 0 ? (_jsx("div", { className: "text-xs text-white py-4 text-center leading-relaxed", children: sidebarMeetingsOnly.length > 0 ? (_jsxs(_Fragment, { children: ["Keine anstehenden Kalender-Termine (ohne Video). Deine n\u00E4chsten Video-Meetings findest du im Block ", _jsx("strong", { className: "text-white", children: "Meetings" }), " darunter \u2014 dort startet der Klick die Konferenz."] })) : (_jsxs(_Fragment, { children: ["Keine anstehenden Termine. Unter ", _jsx("strong", { className: "text-white", children: "Kalender" }), " kannst du welche anlegen \u2014 sie erscheinen hier und im Kalender \u00FCber alle Bereiche."] })) })) : (sidebarAppointmentsOnly.map((ev) => (_jsx(SidebarUpcomingRow, { ev: ev, navigate: navigate }, ev.id)))) })] }), _jsxs(Card, { className: "rounded-3xl border-white/10 bg-white/5 text-white backdrop-blur-xl shrink-0", children: [_jsxs(CardHeader, { children: [_jsx(CardTitle, { className: "text-lg", children: "Meetings" }), _jsx("p", { className: "text-[11px] text-white font-normal mt-1 leading-snug", children: "Videokonferenzen \u2014 Klick \u00F6ffnet das Meeting in einem neuen Tab. Termin-\u00DCbersicht weiterhin im Kalender und links unter Familienkalender." })] }), _jsx(CardContent, { className: "space-y-3", children: !token || !currentUser ? (_jsx("div", { className: "text-xs text-white py-4 text-center", children: "Anmelden, um Meetings zu sehen." })) : sidebarMeetingsOnly.length === 0 ? (_jsx("div", { className: "text-xs text-white py-4 text-center leading-relaxed", children: "Keine anstehenden Video-Meetings. Geplante Meetings erscheinen hier und im Kalender." })) : (sidebarMeetingsOnly.map((ev) => (_jsx(SidebarUpcomingRow, { ev: ev, navigate: navigate }, ev.id)))) })] }), _jsxs(Card, { className: "rounded-3xl border-white/10 bg-white/5 text-white backdrop-blur-xl shrink-0", children: [_jsx(CardHeader, { children: _jsx(CardTitle, { className: "text-lg", children: "Schnellaktionen" }) }), _jsx(CardContent, { className: "grid grid-cols-2 gap-2 sm:gap-3 min-w-0", children: [
                                                                    "Neuer Chat",
                                                                    "Kalender",
                                                                    "Kontakte",
                                                                ...
                                                                }) })] }), _jsx("div", { className: "rounded-[1.35rem] border border-white/10 bg-[#070d18]/90 backdrop-blur-xl p-3 min-w-0 min-h-0 shadow-xl shadow-black/40 shrink-0", children: _jsx(FriendsPanel, { displayName: currentUser?.displayName ?? "", friendCode: currentUser?.friendCode ?? "", activeWorkspaceId: activeWorkspaceId, activeSection: activeSection, workspaceInviteCode: inviteCode, onWorkspaceInviteCodeChange: setInviteCode, onJoinWorkspace: (c) => void joinWithInviteCode(c), generatedWorkspaceInviteCode: generatedInviteCode, onCreateWorkspaceInvite: () => void createInviteCode(), onRegenerateFriendCode: handleRegenerateFriendCode, inviteInfo: inviteInfo, friendInfo: friendInfo, friendSearch: friendSearch, onFriendSearchChange: setFriendSearch, onSearchFriends: () => void searchFriends(), friendSearchResults: friendSearchResults, onSendFriendRequest: (id) => queueFriendRequest(id), incomingFriendRequests: incomingFriendRequests, resolveRequestUser: (id) => usersById[id], onAcceptFriendRequest: (rid) => queueAcceptFriendRequest(rid), onRejectFriendRequest: (rid) => void rejectFriendRequest(rid), friends: friends, friendGroupOptions: friendGroupOptions, onSetFriendGroups: (fid, gs) => void setFriendGroupsForFriend(fid, gs), onAddFriendByCode: addFriendByCode, onOpenFriendProfile: (id) => setPeerProfileUserId(id), onOpenPrivateChat: openPrivateChatWithFriend }) })] })] })] })] })] }), _jsx(NewChatModal, { open: newChatOpen, onClose: () => setNewChatOpen(false), sectionLabel: activeSectionData.label, membersExcludingSelf: membersExcludingSelf, onCreatePrivate: createPrivateChat, onCreateGroup: createGroupChat }), _jsx(ProfileModal, { open: profileModalOpen, onClose: () => setProfileModalOpen(false), activeSectionId: activeSection, activeSectionLabel: activeSectionData.label }), _jsx(PeerProfileModal, { userId: peerProfileUserId, onClose: () => setPeerProfileUserId(null) }), _jsx(FriendCategoryModal, { open: friendCategoryOpen, title: friendCategoryCtx?.mode === "send"
                    ? "Kategorien für den neuen Kontakt"
                    : "Kategorien beim Annehmen", subtitle: friendCategoryCtx?.mode === "send"
                    ? "Wo soll diese Person bei dir erscheinen? Mehrfachauswahl — wird mit der Anfrage gesendet."
                    : "Wo soll der Absender bei dir erscheinen? Mehrfachauswahl — nur für deine Ansicht.", confirmLabel: friendCategoryCtx?.mode === "send" ? "Anfrage senden" : "Annehmen", onCancel: () => {
                    setFriendCategoryOpen(false);
                    setFriendCategoryCtx(null);
