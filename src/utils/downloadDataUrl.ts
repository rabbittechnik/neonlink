...
export function mediaExtFromDataUrl(dataUrl: string): string {
  if (isPdfDataUrl(dataUrl)) return "pdf";
  return imageExtFromDataUrl(dataUrl);
}

export function slugifyDownloadBase(name: string): string {
  const s = name
    .trim()
    .replace(/\s+/g, "_")
    .replace(/[^\w\-äöüÄÖÜß.]+/g, "")
 ...
 */
export async function downloadDataUrlsStaggered(
