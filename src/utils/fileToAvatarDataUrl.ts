...
export function fileToAvatarDataUrl(file: File, maxEdge = 256, quality = 0.82): Promise<string> {
  const toJpeg = (source: CanvasImageSource, width: number, height: number): string => {
...
function fileToAvatarDataUrlLegacy(
  file: File,
  maxEdge: number,
  quality: number,
  toJpeg: (source: CanvasImageSource, width: number, height: number) => string
): Promise<string> {
