...
import { ArrowLeft, Briefcase, CalendarDays, ChevronLeft, ChevronRight, Clock3, Flame, FolderKanban, GraduationCap, HeartHandshake, Home, Loader2, Palmtree, Plus, Trash2, Users, Video, } from "lucide-react";
...
const FAMILY_GRID_MEMBER_THEMES = [
    {
        column: "border-l-[3px] border-l-fuchsia-400/55 bg-[linear-gradient(90deg,rgba(217,70,239,0.12)_0%,rgba(217,70,239,0.02)_60%,transparent_100%)]",
        header: "bg-fuchsia-500/22 text-fuchsia-50 border-b border-fuchsia-400/28 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]",
        dayRow: "bg-fuchsia-500/[0.045]",
    },
    {
        column: "border-l-[3px] border-l-sky-400/55 bg-[linear-gradient(90deg,rgba(14,165,233,0.12)_0%,rgba(14,165,233,0.02)_60%,transparent_100%)]",
        header: "bg-sky-500/22 text-sky-50 border-b border-sky-400/28 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]",
        dayRow: "bg-sky-500/[0.045]",
    },
    {
        column: "border-l-[3px] border-l-emerald-400/55 bg-[linear-gradient(90deg,rgba(52,211,153,0.12)_0%,rgba(52,211,153,0.02)_60%,transparent_100%)]",
        header: "bg-emerald-500/22 text-emerald-50 border-b border-emerald-400/28 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]",
        dayRow: "bg-emerald-500/[0.045]",
    },
    {
        column: "border-l-[3px] border-l-amber-400/55 bg-[linear-gradient(90deg,rgba(251,191,36,0.12)_0%,rgba(251,191,36,0.02)_60%,transparent_100%)]",
        header: "bg-amber-500/22 text-amber-50 border-b border-amber-400/28 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]",
        dayRow: "bg-amber-500/[0.045]",
    },
    {
        column: "border-l-[3px] border-l-rose-400/55 bg-[linear-gradient(90deg,rgba(251,113,133,0.12)_0%,rgba(251,113,133,0.02)_60%,transparent_100%)]",
        header: "bg-rose-500/22 text-rose-50 border-b border-rose-400/28 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]",
        dayRow: "bg-rose-500/[0.045]",
    },
    {
        column: "border-l-[3px] border-l-violet-400/55 bg-[linear-gradient(90deg,rgba(167,139,250,0.12)_0%,rgba(167,139,250,0.02)_60%,transparent_100%)]",
        header: "bg-violet-500/22 text-violet-50 border-b border-violet-400/28 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]",
        dayRow: "bg-violet-500/[0.045]",
    },
];
function familyGridColumnTheme(colId, colIndex) {
    if (colId === COL_GEMEINSAM) {
        return {
            column: "border-l-[3px] border-l-cyan-400/55 bg-[linear-gradient(90deg,rgba(6,182,212,0.14)_0%,rgba(6,182,212,0.03)_60%,transparent_100%)]",
            header: "bg-cyan-500/22 text-cyan-50 border-b border-cyan-400/28 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]",
            dayRow: "bg-cyan-500/[0.045]",
        };
    }
    if (colId === COL_GETEILT) {
        return {
            column: "border-l-[3px] border-l-slate-500/50 bg-[linear-gradient(90deg,rgba(100,116,139,0.14)_0%,rgba(100,116,139,0.03)_60%,transparent_100%)]",
            header: "bg-slate-600/25 text-slate-100 border-b border-slate-400/25 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]",
...
function memberColumnId(userId) {
    return `${COL_MEMBER_PREFIX}${userId}`;
}
function slotLabelIsGemeinsam(label) {
...
function memberColumnIdOrGemeinsam(userId, members) {
    const m = members.find((x) => x.userId === userId);
    if (m && slotLabelIsGemeinsam(m.displayName))
...
function eventStartsOnDay(ev, d) {
...
function formatEventTimeLabel(ev) {
    ...
    if (Number.isNaN(start.getTime()))
        return "";
    const fmt = (x) => x.toLocaleTimeString("de-DE", {
        hour: "2-digit",
        minute: "2-digit",
    });
...
function assignColumn(ev, slots, members) {
    const memberIds = new Set(members.map((m) => m.userId));
    const uidFromSlotId = (slotId) => {
        if (!slotId || slotId === FAMILY_CALENDAR_SELF_SLOT_ID)
            return undefined;
        const s = slots.find((x) => x.id === slotId);
        if (s && memberIds.has(s.ownerUserId) && !slotLabelIsGemeinsam(s.label))
    ...
    if (uid)
        return memberColumnIdOrGemeinsam(uid, members);
    ...
    if (label && label !== "gemeinsam") {
        ...
        if (mem)
            return memberColumnIdOrGemeinsam(mem.userId, members);
...
function assignColumns(ev, slots, members) {
    const cols = new Set();
...
function familySlotOwnerLabel(ev, members, selfColumnFallback) {
    if (!ev.familySlotId)
        return "Gemeinsam";
    if (ev.familySlotId === FAMILY_CALENDAR_SELF_SLOT_ID) {
        return (members.find((m) => m.userId === ev.createdByUserId)?.displayName ??
            ev.familySlotLabel ??
            selfColumnFallback);
    }
...
export default function CalendarPage() {
    ...
    const memberIdsKey = useMemo(() => members
        .map((m) => m.userId)
        .slice()
        .sort()
        .join(","), [members]);
    ...
    const membersForFamilyGrid = useMemo(() => membersSorted.filter((m) => !slotLabelIsGemeinsam(m.displayName)), [membersSorted]);
    ...
    return (_jsxs("div", { className: "min-h-screen w-full bg-[#050816] text-white overflow-x-hidden", children: [_jsx("div", { className: "absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(34,211,238,0.14),transparent_35%),radial-gradient(circle_at_80%_30%,rgba(168,85,247,0.12),transparent_30%)]" }), _jsxs("div", { className: "relative z-10 max-w-6xl mx-auto px-4 py-8 pb-24", children: [_jsxs("div", { className: "flex flex-wrap items-center gap-4 mb-6", children: [_jsxs(Link, { to: "/", className: "inline-flex items-center gap-2 text-sm text-cyan-300 hover:text-cyan-100", children: [_jsx(ArrowLeft, { className: "h-4 w-4" }), "Zur\u00FCck zur App"] }), _jsxs("div", { className: "flex items-center gap-2 text-sky-200", children: [_jsx(CalendarDays, { className: "h-6 w-6" }), _jsx("h1", { className: "text-2xl font-semibold tracking-tight", children: "Kalender" })] })] }), _jsxs("p", { className: "text-sm text-white/60 max-w-3xl mb-6", children: ["Hier siehst du ", _jsx("strong", { children: "alle Termine" }), ", die f\u00FCr dich in ", _jsx("strong", { children: "allen Workspaces" }), " sichtbar sind (z.\u00A0B. Feuerwehr-\u00DCbung im Familien-Kalender mit Rubrik-Farbe). Neu anlegen tust du im gew\u00E4hlten Workspace unten. Pro Termin w\u00E4hlst du die ", _jsx("strong", { children: "Rubrik" }), " (Farbe & Symbol) und optional, bei wem der Eintrag", " ", _jsx("strong", { children: "mit angezeigt" }), " wird. Im ", _jsx("strong", { children: "Familienkalender" }), " gibt es eine Spalte", " ", _jsx("strong", { children: "Gemeinsam" }), " und ", _jsx("strong", { children: "automatisch eine Namensspalte pro Workspace-Mitglied" }), " \u2014 neue Familienmitglieder erscheinen dort, sobald sie dem gemeinsamen Bereich beitreten.", " ", _jsx("strong", { children: "Urlaub" }), "/", _jsx("strong", { children: "Ferien" }), " als durchgehender Balken; in der ", _jsx("strong", { children: "Monatsliste" }), " ", "bei Urlaub/Ferien ", _jsx("strong", { children: "Wer:" }), " (Spalte oder Gemeinsam)."] }), loading ? (_jsx(Loader2, { className: "h-8 w-8 animate-spin text-cyan-400" })) : (_jsxs(_Fragment, { children: [_jsxs("div", { className: "flex flex-wrap gap-3 mb-6 items-end", children: [_jsxs("div", { children: [_jsx("label", { className: "block text-xs text-white/50 mb-1", children: "Workspace" }), _jsx("select", { className: SELECT_DARK, value: workspaceId ?? "", onChange: (e) => setWorkspaceId(e.target.value || null), children: workspaces.map((w) => (_jsx("option", { value: w.id, className: OPTION_DARK, children: w.name }, w.id))) })] }), _jsxs("div", { className: "flex rounded-xl border border-white/15 p-1 bg-black/20", children: [_jsx("button", { type: "button", onClick: () => setView("liste"), className: `px-4 py-2 rounded-lg text-sm ${view === "liste" ? "bg-sky-500/25 text-sky-100" : "text-white/55"}`, children: "Monatsliste" }), _jsx("button", { type: "button", onClick: () => setView("familie"), className: `px-4 py-2 rounded-lg text-sm ${view === "familie" ? "bg-fuchsia-500/25 text-fuchsia-100" : "text-white/55"}`, children: "Familien-Spalten" })] }), _jsxs("div", { className: "flex items-center gap-2 rounded-xl border border-white/15 px-2 py-1 bg-black/20", children: [_jsx("button", { type: "button", "aria-label": "Vorheriger Monat", className: "p-2 rounded-lg hover:bg-white/10", onClick: () => setCursor((c) => (c.m <= 0 ? { y: c.y - 1, m: 11 } : { y: c.y, m: c.m - 1 })), children: _jsx(ChevronLeft, { className: "h-5 w-5" }) }), _jsx("span", { className: "text-sm font-medium min-w-[10rem] text-center capitalize", children: monthTitle }), _jsx("button", { type: "button", "aria-label": "N\u00E4chster Monat", className: "p-2 rounded-lg hover:bg-white/10", onClick: () => setCursor((c) => (c.m >= 11 ? { y: c.y + 1, m: 0 } : { y: c.y, m: c.m + 1 })), children: _jsx(ChevronRight, { className: "h-5 w-5" }) })] }), _jsxs(Button, { type: "button", onClick: openNewModal, className: "rounded-xl bg-gradient-to-r from-sky-500/30 to-violet-500/25 border border-sky-400/30 ml-auto", children: [_jsx(Plus, { className: "h-4 w-4 mr-2" }), "Termin \u00B7 Urlaub \u00B7 Ferien"] })] }), listError ? _jsx("p", { className: "text-sm text-red-300 mb-4", children: listError }) : null, view === "liste" ? (_jsxs("div", { className: "space-y-4", children: [days.map((d) => {
                                        ...
                                        return (_jsxs("div", { className: "rounded-2xl border border-white/10 bg-white/5 p-4", children: [_jsx("div", { className: "text-sm text-white/50 mb-2", children: d.toLocaleDateString("de-DE", { weekday: "long", day: "numeric", month: "long" }) }), _jsx("div", { className: "space-y-2", children: dayEvents.map((ev) => {
                                    ...
                                    }), events.length === 0 ? (_jsx("div", { className: "rounded-2xl border border-white/10 bg-white/5 p-8 text-center text-white/50", children: "Keine Termine in diesem Monat." })) : null] })) : (_jsx("div", { className: "overflow-x-auto rounded-2xl border border-white/10 bg-white/5 text-sm", children: _jsxs("div", { className: "flex min-w-[720px] w-full", children: [_jsxs("div", { className: "sticky left-0 z-30 flex flex-col w-36 shrink-0 border-r border-white/10 border-l-[3px] border-l-slate-400/35 bg-[linear-gradient(90deg,rgba(148,163,184,0.1)_0%,transparent_65%),rgb(10,16,32)]", children: [_jsx("div", { className: "h-10 flex items-center px-2 border-b border-white/10 text-slate-200/90 text-xs font-medium bg-white/[0.04]", children: "Tag" }), days.map((d) => (_jsx("div", { className: "h-[3.5rem] shrink-0 flex items-center px-2 border-b border-white/5 text-white/70 text-xs whitespace-nowrap bg-slate-500/[0.03]", children: d.toLocaleDateString("de-DE", { weekday: "short", day: "numeric", month: "short" }) }, dateKeyLocal(d))))] }), columns.map((col, colIdx) => {
                                            ...
                                            const vacationBars = rangeBarsSource
                                                ...
                                                .filter(Boolean);
                                            const spanAppointmentSource = events.filter((ev) => !isCalendarRangeKind(ev.kind) &&
                                            ...
                                            const spanAppointments = spanAppointmentSource
                                                ...
                                                .filter(Boolean);
                                            return (_jsxs("div", { className: `flex-1 min-w-[7rem] shrink-0 flex flex-col ${colTheme.column}`, children: [_jsx("div", { className: `h-10 flex items-center justify-center px-1 text-xs font-medium text-center ${colTheme.header}`, children: col.label }), _jsxs("div", { className: "relative flex flex-col", children: [vacationBars.map(({ ev, span }) => {
                                                                ...
                                                                const meta = CALENDAR_SECTION_META[ev.sectionId] ?? CALENDAR_SECTION_META.familie;
                                                                const Icon = SECTION_ICONS[ev.sectionId] ?? Home;
                                                                return (_jsx("button", { type: "button", onClick: () => canEdit && openEdit(ev), className: `absolute left-0.5 right-0.5 rounded-md z-10 text-left px-1 py-0.5 border ${meta.border} ${meta.chip} ${canEdit ? "cursor-pointer hover:bg-white/15" : "cursor-default opacity-95"}`, style: {
                                                                        top: `calc(${span.start} * ${FAMILY_GRID_ROW_H})`,
                                                                        height: `calc(${rows} * ${FAMILY_GRID_ROW_H})`,
                                                                    }, children: _jsxs("span", { className: "flex items-start gap-0.5 text-[10px] font-medium leading-tight line-clamp-3", children: [ev.kind === "meeting" ? (_jsx(Video, { className: "h-3 w-3 shrink-0 mt-0.5 text-cyan-300 opacity-90" })) : (_jsx(Icon, { className: "h-3 w-3 shrink-0 mt-0.5 opacity-85" })), ev.title] }) }, `span-${ev.id}`));
                                                            }), days.map((d) => {
                                                                const key = dateKeyLocal(d);
                                                                ...
                                                                return (_jsx("div", { className: `h-[3.5rem] shrink-0 border-b border-white/5 relative pointer-events-none ${colTheme.dayRow}`, children: _jsxs("div", { className: "absolute inset-0 p-1 flex flex-col gap-1 z-20 pointer-events-auto", children: [normalCell.map((ev) => {
                                                                                const meta = CALENDAR_SECTION_META[ev.sectionId] ?? CALENDAR_SECTION_META.familie;
                                                                                const Icon = SECTION_ICONS[ev.sectionId] ?? Home;
                                                                                const canEdit = ev.createdByUserId === mine && ev.kind !== "meeting";
                                                                            ...
                                                                            }), compactCell.length > 0 ? (_jsxs("div", { className: "mt-auto inline-flex items-center gap-1 self-start rounded-md border border-slate-300/25 bg-slate-800/65 px-1.5 py-0.5 text-[10px] text-slate-100", children: [_jsx(Clock3, { className: "h-3 w-3" }), compactCell.length, " Schicht", compactCell.length > 1 ? "en" : ""] })) : null] }) }, key));
                                                            })] })] }, col.id));
                                        ...
                                        : "text-amber-200/80 bg-amber-500/10 border-amber-400/20"}`, children: formKind === "ferien" ? (_jsxs(_Fragment, { children: ["Titel automatisch: ", _jsx("strong", { children: "Gemeinsam" }), " / deine Spalte \u2192 ", _jsx("strong", { children: "Ferien" }), ", sonst", " ", _jsx("strong", { children: "Ferien von \u2026" }), " (Name der gew\u00E4hlten Personenspalte)."] })) : (_jsxs(_Fragment, { children: ["Titel automatisch: ", _jsx("strong", { children: "Gemeinsam" }), " / deine Spalte \u2192 ", _jsx("strong", { children: "Urlaub" }), ", sonst", " ", _jsx("strong", { children: "Urlaub von \u2026" })] })) })), _jsxs("label", { className: "flex items-center gap-2 text-sm cursor-pointer", children: [_jsx("input", { type: "checkbox", checked: formAllDay, onChange: (e) => {
                                                ...
                                                ? "Spalte (Person)"
                                                : "Spalte im Familienkalender (optional)" }), _jsxs("select", { className: `mt-1 w-full ${SELECT_DARK}`, value: formSlotId, onChange: (e) => {
                                                ...
                                                if (formLegacySlot && e.target.value !== formLegacySlot.id)
                                                    setFormLegacySlot(null);
                                                        ...
                                                        : "Gemeinsam" }), membersSorted.map((m) => {
                                                    const slot = slots.find((s) => s.ownerUserId === m.userId && !slotLabelIsGemeinsam(s.label));
                                                    const value = slot?.id ?? (m.userId === mine ? FAMILY_CALENDAR_SELF_SLOT_ID : "");
                                                    if (!value)
                                                        return null;
                                                    return (_jsx("option", { value: value, className: OPTION_DARK, children: m.displayName }, m.userId));
                                                }), formLegacySlot ? (_jsxs("option", { value: formLegacySlot.id, className: OPTION_DARK, children: [formLegacySlot.label, " (bisherige Zuordnung)"] })) : null] }), _jsx("p", { className: "text-[11px] text-white/40 mt-1", children: "Eine Spalte pro Workspace-Mitglied \u2014 neue Mitglieder erscheinen automatisch, sobald sie dem Bereich beitreten." })] }), formSection === "familie" ? (_jsx("p", { className: "text-xs text-cyan-200/80 border border-cyan-400/20 rounded-lg px-2 py-1.5", children: "Familie: Sichtbarkeit ist automatisch f\u00FCr alle Mitglieder aktiv. Die Auswahl unten ist nur \u201EWer ist mit dabei?\u201C." })) : null, _jsxs("div", { children: [_jsx("label", { className: "text-xs text-white/55", children: "Wer ist mit dabei? (Workspace-Mitglieder)" }), _jsxs("div", { className: "mt-2 space-y-2 max-h-32 overflow-y-auto", children: [members
                                                    .filter((m) => m.userId !== mine)
                                                    .map((m) => (_jsxs("label", { className: "flex items-center gap-2 text-sm", children: [_jsx("input", { type: "checkbox", checked: Boolean(formVis[m.userId]), onChange: (e) => setFormVis((p) => ({ ...p, [m.userId]: e.target.checked })) }), m.displayName] }, m.userId))), members.filter((m) => m.userId !== mine).length === 0 ? (_jsx("span", { className: "text-xs text-white/45", children: "Keine weiteren Mitglieder." })) : null] })] }), formKind === "appointment" ? (_jsxs("label", { className: "flex items-center gap-2 text-sm cursor-pointer", children: [_jsx("input", { type: "checkbox", checked: formShiftCompact, onChange: (e) => setFormShiftCompact(e.target.checked), className: "rounded" }), "Als leichte Schicht anzeigen (kompakt im Familienkalender, nicht bei \u201EN\u00E4chste Termine\u201C)"] })) : null, _jsxs("div", { className: "flex flex-wrap gap-2 pt-2 items-center", children: [editId ? (_jsxs(Button, { type: "button", variant: "ghost", disabled: savingEvent, onClick: () => void deleteEditingEvent(), className: "rounded-xl border border-red-400/30 text-red-300 hover:bg-red-500/15", children: [_jsx(Trash2, { className: "h-4 w-4 mr-2" }), "L\u00F6schen"] })) : null, _jsx(Button, { type: "button", disabled: savingEvent || (formKind === "appointment" && !formTitle.trim()), onClick: () => void submitEvent(), className: "flex-1 min-w-[8rem] rounded-xl bg-sky-500/25 border border-sky-400/35", children: savingEvent ? _jsx(Loader2, { className: "h-4 w-4 animate-spin" }) : "Speichern" }), _jsx(Button, { type: "button", variant: "ghost", onClick: () => setModalOpen(false), children: "Abbrechen" })] })] })] }) })) : null] }));
}
