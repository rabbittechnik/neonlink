...
function tryNormalizeStop(raw) {
    ...
    if (typeof o.id !== "string" || typeof o.name !== "string")
        return null;
    const provider = o.provider === "bvg" || o.provider === "db"
        ? o.provider
        : /^9\d{5,}/.test(o.id)
            ? "bvg"
            : "db";
...
export function DeparturesDashboard() {
    ...
    const requestNearby = useCallback(() => {
        ...
        navigator.geolocation.getCurrentPosition(async (pos) => {
            ...
            const useApi = sourceMode === "auto" ? inferred : sourceMode === "bvg" ? "bvg" : "db";
            try {
                const list = useApi === "bvg"
