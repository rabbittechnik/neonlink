        ...
        >
          ...
          >
            <div className="flex items-center justify-between border-b border-white/10 px-5 py-3">
              <h2 id="peer-profile-title" className="text-sm font-semibold tracking-wide text-white/90">
                Profil
              </h2>
              <Button
                type="button"
                variant="ghost"
                className="h-9 w-9 rounded-full text-white/70 hover:text-white hover:bg-white/10 p-0"
                onClick={onClose}
              >
                <X className="h-4 w-4" />
              </Button>
            ...
            <div className="p-5 max-h-[min(70vh,520px)] overflow-y-auto">
              {loading ? (
                ...
                <div className="space-y-4">
                  {isPrivate ? (
                    <>
                      <div className="text-center text-base font-medium text-white/80">{data.displayName}</div>
                      <p className="text-sm text-white/65 text-center py-2 leading-relaxed">
                    ...
                    <>
                      <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="text-xs text-white/50 mt-0.5">{statusLabel(data.status)}</div>
                          {data.friendCode ? (
                            <div className="text-[11px] text-white/40 mt-1 font-mono">Code: {data.friendCode}</div>
                          ) : null}
                        </div>
                      </div>

                      {data.statusMessage?.trim() ? (
                        <div className="rounded-2xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white/85">
                          {data.statusMessage.trim()}
                        </div>
                      ) : null}
                      ...
                      {data.bio?.trim() ? (
                        <div>
                          <div className="text-[10px] uppercase tracking-wider text-white/40 mb-1">Über mich</div>
                          <p className="text-sm text-white/80 whitespace-pre-wrap leading-relaxed">{data.bio.trim()}</p>
                        </div>
                      ) : null}

                      {data.contactEmail ? (
                        <div className="flex items-start gap-2 rounded-2xl border border-white/10 bg-white/5 px-3 py-2 text-sm">
                          <Mail className="h-4 w-4 text-cyan-300/80 shrink-0 mt-0.5" />
                          <div className="min-w-0">
                            <div className="truncate text-white/90">{data.contactEmail}</div>
                      ...
                      {data.phoneMasked ? (
                        <div className="flex items-start gap-2 rounded-2xl border border-white/10 bg-white/5 px-3 py-2 text-sm">
                          <Phone className="h-4 w-4 text-cyan-300/80 shrink-0 mt-0.5" />
                          <div className="min-w-0">
                            <div className="text-white/90">{data.phoneMasked}</div>
                  ...
                  {isPrivate ? (
                    <div className="flex justify-center pt-2">
                      <User className="h-10 w-10 text-white/20" aria-hidden />
                    </div>
                  ) : null}
                </div>
              ) : null}
